  ...
  return (
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Content
        ...
        className={cn(
          'text-primary animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 w-fit origin-(--radix-tooltip-content-transform-origin) rounded-md bg-white px-3 py-1.5 text-xs text-balance shadow shadow-blue-500/5 backdrop-blur',
          className,
        )}
        {...props}
      >
        <div className="z-10 text-xs">{children}</div>
        <TooltipPrimitive.Arrow className="z-0 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] bg-white fill-white backdrop-blur" />
      </TooltipPrimitive.Content>
    </TooltipPrimitive.Portal>
  );
}
