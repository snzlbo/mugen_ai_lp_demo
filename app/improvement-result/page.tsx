...
export default function ImprovementProposal() {
  ...
  return (
    <div className="flex flex-col space-y-10">
      ...
      <AnalysisTable />
      <div className="rounded-xl bg-[#edf6fd] shadow">
        <Tabs
          value={tab}
          ...
          <TabsList className="w-full justify-between p-0">
            {tabs.map((name) => (
              <TabsTrigger
                key={name}
                value={name}
                className={`bg-[#edf6fd] px-7 py-2 font-medium ${
                  tab === name
                    ? 'border-x-0 border-t-0 border-b-2 border-blue-400 bg-[#edf6fd] text-black shadow-none'
                    : 'text-gray-400'
                } rounded-none`}
              >
                {name}
          ...
          <table className="mt-12 w-full border-collapse border border-[#e4e7ec] text-sm">
            <tbody className="text-xs">
              ...
              <tr>
                ...
                <td className="border border-[#e4e7ec] p-3 align-middle">
                  <span className="align-middle text-2xl text-yellow-400">
                    ★
                  </span>
